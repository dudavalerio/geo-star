...
class TelaInicial:
  ...
  def __init__(self):
    ...
    for letra in self.inicial:
      texto_atual += letra
      pyxel.cls(0)
      pyxel.text(120, 130, texto_atual, 7)
    ...
    while time.time() - tempo_inicio < 3:
      pyxel.cls(0)
      pyxel.text(120, 130, self.inicial, 7)
      pyxel.text(100, 160, "Pressione ENTER para continuar", 13)
      pyxel.flip()
      if pyxel.btnp(pyxel.KEY_RETURN) or pyxel.btnp(pyxel.KEY_KP_ENTER):
        break
...
class Jogo:
  ...
  def __init__(self):
    # O pyxel.init já foi chamado na TelaInicial, reaproveitamos a janela existente
    self.texto_1 = ""  # Guarda o input de 'a'
    self.texto_2 = ""  # Guarda o input de 'b'
    self.foco_input = (  # Controla em qual caixa o jogador está digitando
        "a"
    )
    ...
    for estrela in self.estrelas:
      if not estrela.coletada:
        ...
        if distancia <= 0.3:
          ...
          print(
              f"⭐ Estrela na coordenada ({estrela.x}, {estrela.y}) capturada!"
          )
  ...
  def draw(self):
    ...
    cor_borda_a = (
        11 if self.foco_input == "a" else 5
    )  # Fica verde (11) se estiver ativa
    pyxel.rectb(8, 22, 70, 20, cor_borda_a)

    # Caixa de Input do 'b'
    pyxel.text(10, 52, "Valor de 'b' (intercepto Y):", 7)
    cor_borda_b = (
        11 if self.foco_input == "b" else 5
    )  # Fica verde (11) se estiver ativa
    ...
    pyxel.text(
        14, 29, self.texto_1 + (cursor if self.foco_input == "a" else ""), 7
    )
    pyxel.text(
        14, 69, self.texto_2 + (cursor if self.foco_input == "b" else ""), 7
    )
...
# Executa a tela inicial gráfica e depois inicia o jogo
TelaInicial()
Jogo()
